---
title: "10-31-16_derivative_lib_analysis"
output:
  html_document:
    toc: true
    toc_float: true
...
ggplot(dat, aes(total_old_lib / 1e6, total_derived_lib / 1e6, colour = derived_selected_cells)) + 
  geom_point(size = 2) + 
...
ggsave("reads_per_barcode_scatterplot_derived.pdf")

ggplot(dat, aes(total_old_lib / 1e6, total_biotin_derived_lib / 1e6, colour = biotin_selected_cells)) + 
  geom_point(size = 2) + 
  xlab("original library\nreads count (millions)") +
  ylab("biotin derivative library\nreads count (millions") +
  ggtitle("Raw reads associated with each cell barcode") +
  scale_colour_manual(name = "Cell Selected\nfor reamplification\n",
                      values = color_palette) +
  theme_cowplot(font_size = 16, line_size = 1) +
...
ggplot(dat, aes(total_derived_lib / 1e6, total_biotin_derived_lib / 1e6, colour = biotin_selected_cells)) + 
  geom_point(size = 2) + 
  xlab("derivative library\nreads count (millions") +
  ylab("biotin derivative library\nreads count (millions") +
  ggtitle("Raw reads associated with each cell barcode") +
  scale_colour_manual(name = "Cell Selected\nfor reamplification\n",
                      values = color_palette) +
...
ggplot(dat, aes(Barcode, log2ratio_raw_reads_derived, colour = derived_selected_cells)) + 
  geom_point() + 
...
ggsave("reads_ratio_per_barcode_scatterplot_dervied.pdf", width = 7, height = 7)

ggplot(dat, aes(Barcode, log2ratio_raw_reads_biotin, colour = biotin_selected_cells)) + 
  geom_point() + 
  ylab(expression(paste("reads ratio ", frac("biotin library", "original library"), " Log2"))) +
  ggtitle("Enrichment of cell barcodes in derivative library") +  
  scale_colour_manual(name = "Cell Selected\nfor reamplification\n", values = color_palette) +
  theme_cowplot(font_size = 16, line_size = 1) +
  theme(
...
ggsave("reads_ratio_per_barcode_scatterplot_biotin.pdf", width = 7, height = 7)
```

  
  
```{r boxplot_ratio_reads_counts}
ggplot(dat, aes(derived_selected_cells, 
                log2ratio_raw_reads_derived, fill = derived_selected_cells)) + 
  geom_boxplot() + 
  xlab("Selected for Reamplification") +
  ylab(expression(paste("reads ratio ", 
                        frac("derived library", "original library"), " Log2"))) +
  ggtitle("Enrichment of cell barcodes in derivative library") +  
...
ggsave("reads_ratio_per_barcode_boxplot_derived.pdf", width = 7, height = 7)

ggplot(dat, aes(biotin_selected_cells, 
                log2ratio_raw_reads_biotin, fill = biotin_selected_cells)) + 
  geom_boxplot() + 
  xlab("Selected for Reamplification") +
  ylab(expression(paste("reads ratio ", frac("biotin library", "original library"), " Log2"))) +
  ggtitle("Enrichment of cell barcodes in biotin library") +  
  scale_fill_manual(name = "Cell Selected\nfor reamplification\n",
                    values = color_palette) +
  theme_cowplot(font_size = 16, line_size = 1) +
  theme(
...
ggsave("reads_ratio_per_barcode_boxplot_biotin.pdf", width = 7, height = 7)
```

```{r  total_lib_enrichment}
total_biotin <- sum(dat$total_biotin_derived_lib)
total_new <- sum(dat$total_derived_lib)
total_old <- sum(dat$total_old_lib)
dat_group_derived <- group_by(dat, derived_selected_cells) %>% 
  summarize(total_new = sum(total_derived_lib) / total_new, 
            total_old = sum(total_old_lib) / total_old) %>% 
  gather(lib, percent_lib, -derived_selected_cells ) %>%
  mutate(lib = factor(lib, levels = c("total_new", "total_old"), 
                      labels = c("amplified library", "original library")),
         selected_cells = derived_selected_cells,
         expt = "Normal") %>%
  select(-derived_selected_cells)

dat_group_biotin <- group_by(dat, biotin_selected_cells) %>% 
  summarize(total_new = sum(total_biotin_derived_lib) / total_biotin, 
            total_old = sum(total_old_lib) / total_old) %>% 
  gather(lib, percent_lib, -biotin_selected_cells ) %>%
  mutate(lib = factor(lib, levels = c("total_new", "total_old"), 
                      labels = c("amplified library", "original library")),
         selected_cells = biotin_selected_cells,
         expt = "Biotin") %>%
  select(-biotin_selected_cells)

dat_group <- bind_rows(dat_group_derived, dat_group_biotin) 
...
dat_group %>% group_by(expt) %>% mutate(enrichment_over_unamplified = nth(percent_lib, 1) / nth(percent_lib, 3)) %>% knitr::kable(title = "Enrichment of reads over unamplified")
```
...
```{r find_barcodes_close_to_amplified_libs, eval = F}
similar_seq_barcodes <- dat %>% 
  filter(derived_selected_cells == "Selected for\nreamplification") %>%
    select(min_barcodes) %>% 
...
system("mkdir -p ../../data/biotinylated_libs/count_matrices/")

in_dir <- "../../data/derivative_libs/star/feature_counts/"
pattern <- "\\d+.tsv$"
derivative_countdat <- read_counts(in_dir, pattern)
write.table(derivative_countdat, "../../data/derivative_libs/count_matrices/star_reads_counts.txt", quote = F, sep = "\t")

in_dir <- "../../data/biotinylated_libs/star/feature_counts/"
pattern <- "\\d+.tsv$"
biotin_countdat <- read_counts(in_dir, pattern)
write.table(biotin_countdat, "../../data/biotinylated_libs/count_matrices/star_reads_counts.txt", quote = F, sep = "\t")
