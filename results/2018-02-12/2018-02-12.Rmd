...
umi <- ggplot(filter(subsampled_metadat, library == "phosphorothioate"), 
...
```{r tSNE_og}
library(Seurat)
seurat_obj <- CreateSeuratObject(sc_objs$original$umi_matrix, project = "og", 
                                 min.genes = 1, min.cells = 1, names.delim = "_", names.field = 2)
seurat_obj <- NormalizeData(seurat_obj)
seurat_obj <- FindVariableGenes(seurat_obj)
```
