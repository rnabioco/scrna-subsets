...
This RMarkdown document uses the following processed data files generated
by the `process.Rmd`:
...
figure | filename  
--------- | -------
Fig. 1CD | umi_scatterplot_human_mouse.pdf 
Fig. 1E | umi_ratio_MA.pdf  
Fig. 1F | cDNA_duplication.pdf 
Fig. 1G | new_genes_species_specific_boxplot.pdf
Fig. 1H | new_umis_species_specific_boxplot.pdf
...
sc_metadat %>% 
  ...
  summarize(read_enrichment = nth(total_reads, 2) / nth(total_reads, 1),
         umi_enrichment = nth(total_umis, 2) / nth(total_umis, 1)) 
...
sc_metadat %>% 
  ...
  group_by(library) %>% 
  summarize(mean_duplication = mean(cDNA_duplication),
            sd_duplication = sd(cDNA_duplication))

sc_metadat %>% 
  filter(library == resampled_lib,
         resampled) %>% 
  dplyr::select(library, cell_id, cDNA_duplication)
```
...
save_plot("new_genes_species_specific_boxplots.pdf", plt, 
...
save_plot("new_umis_species_specific_boxplots.pdf", plt, 
          base_aspect_ratio = 1.1, base_height = 4) 
```
...
mdat_tbl <- sc_objs$original_10x$meta_dat %>% 
   select(barcode_10x, og_purity_umis, resampled) %>% 
  mutate(species = ifelse(og_purity_umis > 0.80,
                          "Human",
                          ifelse(og_purity_umis < 0.20,
                                 "Mouse",
                                 "Multiplet")))
  
mdat <- as.data.frame(mdat_tbl) %>% 
  dplyr::select(-resampled) %>% 
  tibble::column_to_rownames("barcode_10x")
sobj <- AddMetaData(sobj, mdat)
sobj <- SetAllIdent(sobj, "species")

plt_dat <- GetCellEmbeddings(sobj, "tsne") %>% 
  as.data.frame() %>% 
  tibble::rownames_to_column("cell") %>% 
  left_join(mdat_tbl, by = c("cell" = "barcode_10x"))

og_tsne_plt <- ggplot(plt_dat, 
                          aes(tSNE_1, tSNE_2)) + 
...
save_plot("original_cells_tsne.pdf", og_tsne_plt, 
...
save_plot("original_resampled_cells_mouse_human.pdf", plt, 
          base_height = 5, base_aspect_ratio = .9)
...
resampled_cells <- ggplot(tsne_dat, 
                          aes(tSNE_1, tSNE_2)) + 
  geom_point(color = "lightgrey", size = 0.1) + 
...
plt_out <- plot_grid(og_tsne_plt, 
                     plt, 
                     resampled_cells,
                     ncol = 3)
plt_out

save_plot("resampled_tsne.pdf", plt_out, 
          nrow = 1, ncol = 3,
          base_height = 4.5, base_aspect_ratio = 0.9)
```
...
plt_dat <- resampled_metadat %>% 
  ...
  mutate(type = ifelse(str_detect(type, "^og"),
         ...
         library = factor(library, 
                          levels = resampled_libs)) %>% 
  separate(type, c("resampled", 
                   "not_import", 
                   "count_type"), sep = "_") %>% 
  select(-not_import)

plt_dat <- filter(plt_dat, resampled == "Original") %>% 
  mutate(species = ifelse(purity > 0.80, 
                          "Human",
                          ifelse(purity < 0.20,
                                 "Mouse",
                                 "multiplet"))) %>%
  select(-purity, -resampled) %>% 
  left_join(plt_dat, ., by = c("new_id", "library", "count_type")) %>% 
  mutate(purity = ifelse(species == "Mouse",
                         1 - purity,
                         purity))
...
plt_dat %>% 
  spread(resampled, purity) %>% 
  mutate(delta = Original - Resampled)
```
