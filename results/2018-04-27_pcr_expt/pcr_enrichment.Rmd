...
sc_objs <- map(sc_objs, 
           function(x){
             gene_mdata <- map(x$shared_genes, length)
             gene_mdata <- data_frame(cell_id = names(gene_mdata),
                                      shared_genes = unname(unlist(gene_mdata)))
             gene_mdata <- mutate(gene_mdata,
                                  not_recovered_genes = map_int(x$not_recovered_genes,
                                                                length),
                                  new_genes = map_int(x$new_genes, 
                                                      length))
...
genes_recovered %>% 
  dplyr::filter(library == "phosporothioate",
                subsampled)
```
