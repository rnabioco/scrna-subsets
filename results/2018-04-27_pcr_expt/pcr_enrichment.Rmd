...
bc_metadat <- read_tsv(bc_metadat, col_names = c("cell_id", "Barcode"))
```
...
read_dat <- map(read_dat, function(x){
                       ...
                       new_cell_cols <- str_split(cell_cols, ":",
                                                  simplify = T)
...
umi_plots <- map(split(subsampled_metadat, subsampled_metadat$library),
  function(x){
    ggplot(x, 
       aes(og_total_umis, norm_umi_proportion, colour = subsampled)) + 
  geom_point() + 
...
plt <- plot_grid(plotlist = umi_plots, nrow = 2)
save_plot("umi_ratio_MA.pdf", plt, ncol = 2, nrow = 2, base_width =5, base_height = 5)
```
...
dat_group <- group_by(dat, library, subsampled) %>% 
  ...
  mutate(lib = factor(lib, levels = c("total_old", "total_new"), 
                      labels = c("original\nlibrary", "subsampled\nlibrary")),
         subsampled = factor(subsampled, levels = c( "TRUE", "FALSE"))) %>% 
  arrange(subsampled)
...
ggplot(subsampled_metadat, aes(og_human_genes,
                               n_human_genes, colour = subsampled)) + 
  geom_point() + 
...
ggplot(subsampled_metadat, aes(og_mouse_genes,
                               n_mouse_genes,
                               colour = subsampled)) + 
  geom_point() + 
  ylab("subsampled_genes") +
...
umis_recovered <- umis_recovered %>% 
  ...
  mutate(subsampled = ifelse(is.na(subsampled),
                             F,
                             subsampled),
...
shared_genes <- intersect(rownames(sc_objs$phosphorothioate$norm_umi), 
                          rownames(sc_objs$original$norm_umi))
cols <- colnames(sc_objs$phosphorothioate$norm_umi)

ma_dat <- map(unique(unlist(cells$phosphoro)),
    ~calc_ma(sc_objs$phosphorothioate$umi_matrix[shared_genes, cols], 
             sc_objs$original$umi_matrix[shared_genes, cols],
             cell = .x))

ma_dat <- map(ma_dat,
              ~dplyr::filter(.x, mean_expression_log2 != 0))

names(ma_dat) <- unique(unlist(cells$phosphoro))
...
plt <- TSNEPlot(sobj, colors.use = brewer.pal(7, "Set1")) + 
  labs(title = "Breast Cancer PDX") +
  theme(legend.position = "bottom")

sobj <- FindClusters(sobj, dims.use = 1:5, save.SNN = T, print.output = F)
save_plot("original_tsne.pdf", plt, base_height = 4.25, base_width = 4.25)
```
...
```{r sub_tsne}
mat <- sc_objs$original$umi_matrix
good_cell_ids <- sc_objs$original$meta_dat %>% 
  filter(str_detect(cell_id, "1Tumor|2LukGFP|3Brain|4Bone"),
         !str_detect(cell_id, "2LukGFPDead")) %>% 
  pull(cell_id)

mat <- mat[, good_cell_ids]

subsampled_ids <- sc_objs$phosphorothioate$meta_dat %>% 
  filter(!subsampled) %>% 
  pull(cell_id)
...
sobj <- RunPCA(sobj, pc.genes = rownames(sobj@data), pcs.compute = 5)
sobj <- RunTSNE(sobj, dims.use = 1:5, seed.use = 20180427)
...
purity_plt <- FeaturePlot(sobj, 
                        features.plot = "og_purity_reads",
                        no.legend = F, do.return = T) 

subsampled_cells <- TSNEPlot(SetAllIdent(sobj, 
                                         "subsampled"), 
                             colors.use = brewer.pal(7, "Set1")) + 
  labs(title = "Breast Cancer PDX") +
  theme(legend.position = "bottom")

plt <- plot_grid(tsne_plt, purity_plt[[1]], subsampled_cells,
                 nrow = 3, rel_widths = c(1, 1.5, 1))

save_plot("original_tsne_mouse_and_human.pdf", plt, 
          nrow = 3, ncol = 1,
...
subsampled_mat <- sc_objs$phosphorothioate$umi_matrix[, subsampled_ids]
...
new_ids <- sobj@meta.data %>% 
  ...
  mutate(new_id = str_sub(orig.ident, 2),
         ...
         new_id = ifelse(str_detect(new_id, "Tumor"),
                         "Primary\ntumor",
                         new_id),
         subsampled = ifelse(str_detect(cell, "subsampled"),
                             "subsampled",
                             "not subsampled"))
...
subsampled_mdata <- filter(mdata_og, subsampled) %>% 
  select(cell_id, og_purity_reads = purity_reads) %>% 
  mutate(cell_id = str_c(cell_id, "::subsampled"))

mdata_og <- mdata_og %>% 
  select(cell_id, og_purity_reads) %>% 
  bind_rows(., subsampled_mdata) %>% 
  as.data.frame() %>% 
  column_to_rownames("cell_id")
...
subsampled_cell_dat <- tmp[str_c(cells$phosphoro, "::subsampled"), ] %>%
  rownames_to_column("cell") %>% 
...
subsampled_cells_purity <- FeaturePlot(sobj,
                                       features.plot = "og_purity_reads",
                        no.legend = F, 
...
sc_metadat <- arrange(sc_metadat, subsampled)
a <- ggplot(sc_metadat, aes(cell_id, total_reads)) +
  geom_point(aes(color = subsampled), pt.size = 0.75) +
  labs(x = "Cell",
       y = "Sequencing Depth") + 
