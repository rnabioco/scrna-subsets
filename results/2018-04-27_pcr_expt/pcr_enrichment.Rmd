...
species_spec <- plt_dat %>% 
  filter(resampled == "Original", count_type == "UMIs") %>% 
  group_by(new_id, library) %>% 
  mutate(species = ifelse(purity > 0.80, 
                          "Human",
                          ifelse(purity < 0.20,
                                 "Mouse",
                                 "Mixed"))) %>% 
  select(new_id, library, species)

summary_dat <- left_join(plt_dat,
                         species_spec,
                         by = c("new_id", "library"))

summary_dat <- filter(summary_dat, 
                      count_type == "UMIs")
...
summary_dat %>% 
  ...
  mutate(delta_purity = Original - Resampled) %>% 
  group_by(library) %>% 
  summarize(mean(Original), 
            sd(Original),
            mean(Resampled),
            sd(Resampled))
```