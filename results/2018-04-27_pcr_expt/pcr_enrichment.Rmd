...
subsampled_lib <- "phosphorothioate"
```
...
plt <- ggplot(sc_metadat, aes(total_umis, cDNA_duplication)) +
  geom_point(aes(color = subsampled), size = 0.5) +
  labs(x = expression(paste("UMIs (log"[10],")")),
...
plt 
save_plot("cDNA_duplication.pdf", plt, base_height = 5)
```
...
save_plot("umi_ratio_MA.pdf", plt, ncol = 2, nrow = 1, 
...
read_plots <- map(split(subsampled_metadat, subsampled_metadat$library),
  function(x){
  ggplot(x, 
...
genes_to_plot <- rownames(sc_objs[[subsampled_lib]]$umi_matrix)
cols <- colnames(sc_objs[[subsampled_lib]]$norm_umi)
cell_ids <- cells[[subsampled_lib]]
...
plot_scatter <- function(df){
  ...
  ggplot(df, 
         aes_string("original", "subsampled")) +
    geom_point(size = 0.5) + 
...
expr_dat <- map(cell_ids,
  function(x){
    genes_to_plot <- names(sc_objs[[subsampled_lib]]$genes_detected[[x]])
    get_paired_expr(sc_objs[[subsampled_lib]]$umi_matrix[genes_to_plot,
                                                              cols],
            ref_mat[genes_to_plot, cols],
             cell = x)
})
...
names(expr_dat) <- cell_ids
expr_dat <- bind_rows(expr_dat, .id = "cell")
expressed_in_subsampled_plt <- plot_scatter(expr_dat)
expressed_in_subsampled_plt
...
names(expr_dat) <- cell_ids
expr_dat <- bind_rows(expr_dat, .id = "cell")
new_gene_plt <- ggplot(expr_dat, aes(cell,subsampled)) +
  geom_jitter(alpha = 0.55) +
  geom_violin(aes(fill = cell)) +
  ylim(0, max(expr_dat$subsampled) * 1.10) + 
  scale_fill_brewer(palette = "Set1") +
