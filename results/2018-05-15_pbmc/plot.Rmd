...
This RMarkdown document use the following processed data files generated
by the `process.Rmd`:
...
1. ``original_pbmc_markers.txt` marker genes from the original matrix  

And the following text files:

1. `downsampled_mk_cluster_markers_no_resampling.txt.gz` Markers produced with increasing numberd of megakaryocytes. Produced by `2018-09-04_downsampling/downsample.R`.
1. `downsampled_mk_cluster_markers_resampling.txt.gz` Produced by `2018-09-04_downsampling/downsample.R`.

The following figures will be generated:

figure | filename  
--------- | -------
Fig. 2A | original_pbmc_annotated_tsne_arrows.pdf 
Fig. 2B | umi_ratio_MA.pdf  
Fig. 2C | cDNA_duplication 
Fig. 2D | new_genes_barplot.pdf
Fig. 2D | new_umis_barplot.pdf
Fig. 2E-G | resampled_vs_original_closeup.pdf
Fig. 3A | per_cell_MA_seq_saturation_hexplot_with_cluster_medians.pdf
Fig. 3B | nmarkers_kirkpatrick_pbmcs.pdf
Fig. 3C | resampling_marker_discovery_sensitivity.pdf
Fig. 3D | resampling_gene_discovery_sensitivity_with_replication.pdf
...
sc_metadat %>% 
 ...
 sc_metadat %>%
  ...
  sc_metadat %>%
   group_by(library, resampled) %>%
   mutate(total_reads / og_total_reads) %>% 
    filter(resampled)
```
...
plt <- ggplot(plt_dat, 
       aes(type, count, 
           group = resampled)) +
  geom_quasirandom(aes(color = resampled), 
                   method = "tukeyDense", 
                   dodge.width= 1) +
...
hex_plt <- ggplot(plt_dat_hex,
  ...
  scale_fill_gradientn(colours = rev(brewer.pal(9, "RdYlBu")),
                         ...
                         na.value = "lightgrey") +
  facet_wrap(~cell, nrow = 1) +
  labs(x = expression(paste("Average Expression (log"[2], 
                            " UMIs from Original and Resampled)")),  
       y = expression(paste("UMIs ", frac("Resampled","Original"), 
                            " (log"[2], ")"))) +
  theme(legend.position = "top",
        legend.key.width = unit(7.5, 
                                "mm"),
...
```{r}
subsampled_markers_og <- read_tsv(file.path("..",
                                            "2018-09-04_downsampling",                                 "downsampled_mk_cluster_markers_no_resampling.txt.gz")) 

subsampled_markers <- read_tsv(file.path("..",
                                         "2018-09-04_downsampling",                                       "downsampled_mk_cluster_markers_resampling.txt.gz"))

og_summary <- subsampled_markers_og %>% 
  filter(p_val_adj < 0.01) %>% 
  count(replicate, n_mks) %>% 
  group_by(n_mks) %>% 
...
plt <- ggplot(summary_dat, 
  ...
  geom_ribbon(aes(ymin= mean_n - sd_n, 
                  ymax = mean_n + sd_n,
                  fill = expt), 
              alpha = 0.2) +
  scale_color_manual(values = color_palette, name = "") +
  scale_fill_manual(values = color_palette, name = "") +
  ylim(c(0, max(summary_dat$mean_n))) + 
...
new_names <- c("Original Library", "Resampled Library")
names(new_names) <- c("original_genes", "resampled_genes")
plt_dat <- mutate(plt_dat, library = new_names[library])

plt <- ggplot(plt_dat, 
       aes(n_cells, mean_genes)) +
       geom_line(aes(color = library)) +
       geom_ribbon(aes(ymin = mean_genes - sd_genes,
                      ymax = mean_genes + sd_genes,
                      fill = library), 
                   alpha = 0.2) + 
  ylim(c(0, max(plt_dat$mean_genes))) + 
  scale_color_manual(values = color_palette, name = "") +
  scale_fill_manual(values = color_palette, name = "") +
  labs(x = "# of Megakaryocytes in Cluster",
       y = "# of Genes\nDetected in Cluster") +
