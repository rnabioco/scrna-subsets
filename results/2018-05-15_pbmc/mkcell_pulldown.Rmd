...
plt_dat <- genes_recovered %>% 
  ...
  mutate(type = ifelse(type == "new\ngenes",
         ...
         cell_id = factor(cell_id, 
                          levels = c(str_c(cells[[subsampled_lib]],
                                           "-1"), 
                          "Not targeted\nbarcodes\n(mean)")))
...
```{r, eval = F}
resampled_cell_info <- sobj@meta.data %>% 
  tibble::rownames_to_column("cell") 

og_cell_info <- og_sobj@meta.data %>% 
  tibble::rownames_to_column("cell") %>% 
  select(cell, cell_labels)

combined_cell_info <- left_join(resampled_cell_info,
                                og_cell_info,
                                by = "cell") 

combined_cell_info <- mutate(combined_cell_info,
                             cell_labels = as.character(cell_labels),
                             cell_labels = ifelse(is.na(cell_labels) &
                                                   str_detect(cell,
                                                              "::resampled"), 
                                                  "Megakaryocytes",
                                                  cell_labels))

combined_cell_info <- select(combined_cell_info, 
                             cell, cell_labels) %>% 
  as.data.frame() %>% 
  tibble::column_to_rownames("cell")

sobj <- AddMetaData(sobj, combined_cell_info)

## original mk markers

mk_markers <- read_tsv("all_og_markers.txt") %>% 
  filter(cluster == "Megakaryocytes", p_val_adj < 0.01)

## get avg expression with and without resampling
sobj <- SetAllIdent(sobj, "cell_labels")
avg_resampled_expr <- AverageExpression(sobj)
colnames(avg_resampled_expr) <- colnames(avg_resampled_expr) %>% 
  str_c(., " resampled")

og_sobj <- SetAllIdent(og_sobj, "cell_labels")
avg_og_expr <- AverageExpression(og_sobj) 
colnames(avg_og_expr) <- colnames(avg_og_expr) %>% 
  str_c(., " original")

## get all MK cell expression
mks <- rownames(sobj@meta.data)[sobj@meta.data$resampled != "not resampled"]

mk_gene_expr <- FetchData(sobj, 
                          vars.all = mk_markers$gene, 
                          cells.use = mks)
mk_gene_expr <- t(mk_gene_expr)

avg_expr <- cbind(avg_resampled_expr[mk_markers$gene, ], 
                  avg_og_expr[mk_markers$gene, ])
avg_expr <- log1p(avg_expr)
avg_expr <- avg_expr[, str_detect(colnames(avg_expr), "Megakaryocytes")]
all_expr <- cbind(avg_expr,
                  mk_gene_expr[rownames(avg_expr), ])
```
...
plt
save_plot("per_cell_MA_mk_markers.pdf", plt, base_height = 6)
```
...
To determine if the new genes detected in the 4 cells are actually markers of MKs I instead compared these genes to genes identified in another PBMC experiment. 10x genomics has a scRNA-Seq dataset with 68k cells, which has > 200 cells (compared to our 70). I computed markers for these cells and will use this dataset as a reference for MK cell markers. 
...
new_genes <- sc_objs$mkcell_pulldown$new_genes[str_c(cells$mkcell_pulldown,
                                                     "-1")]
all_new_genes <- unique(unlist(new_genes))

genes <- list(all_new_genes = all_new_genes, mk_68k = mk_10x_markers$gene)

all_genes <- unique(unlist(genes))
lgl_df <- map_df(genes, ~all_genes %in% .x) %>% 
  as.data.frame()

fit <- eulerr::euler(lgl_df)
plot(fit)

## Now add in "expressed" not just markers

```


#### scatterplots colored with markers
